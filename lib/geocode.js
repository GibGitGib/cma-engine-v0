...
import { NEIGHBORHOOD_GEOJSON } from './data/ma-neighborhoods.js';

// Neighborhood polygons are imported as a JS module rather than read with fs.
// A cwd-relative fs.readFileSync breaks on serverless: cwd is not guaranteed to
// be the project root, and a non-imported data file is not traced into the
// function bundle, so the read throws at module load in production.
const NEIGHBORHOODS = NEIGHBORHOOD_GEOJSON.features;
...
export async function addressToSubjectProperty(address, options = {}) {
  ...
  // 2-4. Resolve polygon, estimate attributes, build SubjectProperty
  return buildSubjectFromGeo(geo, { propertyType, sqft, beds, baths, yearBuilt, conditionOverride });
}

/**
 * Coordinates → SubjectProperty, with NO geocoding round-trip.
 * Used by the "use my location" flow: the browser already has exact coords, and
 * resolvePolygonId() works directly on lat/lon. Previously the client stuffed a
 * "42.36, -71.06 near Dorchester, MA" string into the address field, which no
 * geocoder can resolve — every such request 500'd.
 */
export function coordsToSubjectProperty(lat, lon, options = {}) {
  const { formattedAddress = null, ...rest } = options;
  const geo = {
    lat,
    lon,
    formattedAddress: formattedAddress || `${lat.toFixed(5)}, ${lon.toFixed(5)}`,
  };
  return buildSubjectFromGeo(geo, rest);
}

/**
 * Shared tail of the pipeline: polygon resolution + attribute estimation.
 */
function buildSubjectFromGeo(geo, options = {}) {
  const {
    propertyType = 'single',
    sqft = null,
    beds = null,
